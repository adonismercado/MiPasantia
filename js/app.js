...
checkboxDescanso.addEventListener(
    "change",
    () => {

        campoDescanso.hidden =
            !checkboxDescanso.checked;
...
inputSalida.addEventListener(
    "input",
    actualizarVistaPrevia
);

inputDescanso.addEventListener(
    "input",
    actualizarVistaPrevia
);
...
function actualizarResumen(
    ...
    const promedio =
        calcularPromedioJornada(
            jornadas
        );

    const estimacion =
        estimarFechaFinalizacion(
            jornadas
        );
    ...
    if (
        ...
        fechaEstimada.textContent =
            formatearFecha(
                estimacion.fecha
            );

        if (
            estimacion.jornadasRestantes ===
            0
        ) {
            detalleEstimacion.textContent =
                "Meta completada";
        } else {
            ...
            detalleEstimacion.textContent =
                `≈ ${estimacion.jornadasRestantes} jornadas restantes · ${dias}`;
        }
        ...
        detalleEstimacion.textContent =
            "Registra jornadas para calcularla";
    }
...
function confirmarEliminarJornada(
    id
) {
    const jornada =
        obtenerJornadaPorId(
